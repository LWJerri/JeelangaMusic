...
module.exports = {
  ...
  execute: async function (client, message, args) {
    ...
    if (
      /http(s):\/\/(www.)?(youtube.[a-z]{0,10}\/((watch\?v=[a-zA-Z0-9]{0,50})|(embed\/[a-zA-Z0-9]{0,50})))|(youtu.be\/{0,50})/.test(
        args.join(""),
      )
    ) {
      /**
      ...
      const addQueueEmbed = new MessageEmbed()
        ...
        .addFields(
          ...
          {
            name: "Playlist time",
            value: `${corePlayer.parseSeconde(allTime)}`,
            inline: true,
          },
        )
      ...
      return message.channel.send(
        youtube.response
          ? youtube.response.data
            ? youtube.response.data.error
              ? youtube.response.data.error.status
              : "undefined"
            : "undefined"
          : "undefined",
        { code: "js" },
