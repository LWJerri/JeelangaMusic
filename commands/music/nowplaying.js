...
module.exports = {
  ...
  execute: async function (client, message, args) {
    ...
    const duration = moment.duration({
      ms: player.queue[player.index].time,
    });
    ...
    const npEmbed = new MessageEmbed()
      ...
      .setDescription(
        `${split(player.queue[player.index].snippet.description, 1000)}\n[${
          player.queue[player.index].snippet.title
        }](https://www.youtube.com/watch?v=${player.queue[player.index].id.videoId})`,
      )
